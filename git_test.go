...
func TestUpdateGitStatus(t *testing.T) {
	...
	count, files, changed, statuses, dirtyDirs, _, _ := ix.UpdateGitStatus()
	...
	count2, _, changed2, statuses2, _, _, _ := ix.UpdateGitStatus()
	if !changed2 {
	...
	_, _, changed3, _, _, _, _ := ix.UpdateGitStatus()
	if changed3 {
...
// tabs stay stale while a harness keeps editing (px0-ai/px0#157).
func TestUpdateGitStatusSeesEditsWithUnchangedStatus(t *testing.T) {
	...
	if _, _, changed, st, _, _, _ := ix.UpdateGitStatus(); !changed || st["keep.go"] != "M" || st["fresh.txt"] != "U" {
	...
	_, _, changed, st, _, _, touched := ix.UpdateGitStatus()
	if !changed {
	...
	if _, _, changed, _, _, _, touched := ix.UpdateGitStatus(); changed || len(touched) != 0 {
...
func TestGitStatusAgainstAndPRDiff(t *testing.T) {
	...
	count, files, _, statuses, _, _, _ := ix.UpdateGitStatus()
	...
	_, _, changed, statuses, _, _, _ := ix.UpdateGitStatus()
