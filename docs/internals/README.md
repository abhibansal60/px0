...
- [Integrated Terminal & Harness Sessions](terminal.md): Pseudo-terminals opened with raw ioctls (no CGO, no dependency), per-session ring buffers with byte-offset cursors, one multiplexed SSE stream with resume and reset, loopback-plus-token access control, lazy-loaded vendored xterm.js, and the Alt+E handoff into a live harness session.
